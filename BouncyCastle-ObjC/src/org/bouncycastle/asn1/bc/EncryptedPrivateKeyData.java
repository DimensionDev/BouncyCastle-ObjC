    ...
    extends ASN1Object
{
    ...
    public EncryptedPrivateKeyData(EncryptedPrivateKeyInfo encryptedPrivateKeyInfo, X509Certificate[] certificateChain)
    {
    ...
    private EncryptedPrivateKeyData(ASN1Sequence seq)
    {
        ...
        certificateChain = new X509Certificate[certSeq.size()];
        for (int i = 0; i != certificateChain.length; i++)
        {
            certificateChain[i] = X509Certificate.getInstance(certSeq.getObjectAt(i));
        }
    }
    ...
    {
        X509Certificate[] tmp = new X509Certificate[certificateChain.length];
