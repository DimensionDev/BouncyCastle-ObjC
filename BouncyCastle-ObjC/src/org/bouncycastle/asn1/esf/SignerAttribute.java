...
import org.bouncycastle.asn1.x509.X509Attribute;
    ...
    extends ASN1Object
{
        ...
        ASN1Sequence seq)
    {
        ...
        for (Enumeration e = seq.getObjects(); e.hasMoreElements();)
        {
            ...
            if (taggedObject.getTagNo() == 0)
            {
                ...
                X509Attribute[]  attributes = new X509Attribute[attrs.size()];

                for (int i = 0; i != attributes.length; i++)
                {
                    attributes[i] = X509Attribute.getInstance(attrs.getObjectAt(i));
                }
    ...
    public SignerAttribute(
        X509Attribute[] claimedAttributes)
    {
     ...
     * type CmsAttribute[] or AttributeCertificate depending on what tag was used.
     *
     ...
     *
     *  ClaimedAttributes ::= SEQUENCE OF CmsAttribute
    ...
    public ASN1Primitive toASN1Primitive()
    {
        ...
        {
            if (values[i] instanceof X509Attribute[])
            {
                v.add(new DERTaggedObject(0, new DERSequence((X509Attribute[])values[i])));
            }
            else
            {
