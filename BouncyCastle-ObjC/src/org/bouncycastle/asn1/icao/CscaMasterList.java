 ...
 *   certList               SET OF X509Certificate }
 *
    ...
    extends ASN1Object
{
    ...
    private X509Certificate[] certList;
        ...
        ASN1Sequence seq)
    {
        ...
        certList = new X509Certificate[certSet.size()];
        for (int i = 0; i < certList.length; i++)
        {
            certList[i]
                = X509Certificate.getInstance(certSet.getObjectAt(i));
        }
    }

    public CscaMasterList(
        X509Certificate[] certStructs)
    {
    ...
    public X509Certificate[] getCertStructs()
    {
        return copyCertList(certList);
    }

    private X509Certificate[] copyCertList(X509Certificate[] orig)
    {
        X509Certificate[] certs = new X509Certificate[orig.length];
