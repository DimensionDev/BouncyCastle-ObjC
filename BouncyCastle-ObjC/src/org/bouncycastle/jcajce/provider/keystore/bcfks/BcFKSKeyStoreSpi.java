...
import org.bouncycastle.asn1.cms.CmsCCMParameters;
    ...
    extends KeyStoreSpi
{
    ...
    public Certificate[] engineGetCertificateChain(String alias)
    {
        ...
        if (ent != null)
        {
            ...
            {
                EncryptedPrivateKeyData encPrivData = EncryptedPrivateKeyData.getInstance(ent.getData());
                org.bouncycastle.asn1.x509.X509Certificate[] certificates = encPrivData.getCertificateChain();
    ...
    private Certificate decodeCertificate(Object cert)
    {
        if (helper != null)
        {
            try
            {
                ...
                return certFact.generateCertificate(new ByteArrayInputStream(org.bouncycastle.asn1.x509.X509Certificate.getInstance(cert).getEncoded()));
            }
            catch (Exception e)
            {
                return null;
        ...
        if (key instanceof PrivateKey)
        {
            ...
            try
            {
                ...
                if (storeEncryptionAlgorithm.equals(NISTObjectIdentifiers.id_aes256_CCM))
                {
                    ...
                    PBES2Parameters pbeParams = new PBES2Parameters(pbkdAlgId, new EncryptionScheme(NISTObjectIdentifiers.id_aes256_CCM, CmsCCMParameters.getInstance(algParams.getEncoded())));
    ...
    {
        org.bouncycastle.asn1.x509.X509Certificate[] certChain = new org.bouncycastle.asn1.x509.X509Certificate[chain.length];
        for (int i = 0; i != chain.length; i++)
        {
            certChain[i] = org.bouncycastle.asn1.x509.X509Certificate.getInstance(chain[i].getEncoded());
        }
        ...
        else if (parameter instanceof BCFKSLoadStoreParameter)
        {
            ...
            if (bcParam.getStoreSignatureKey() != null)
            {
                ...
                try
                {
                    ...
                    if (certs != null)
                    {
                        org.bouncycastle.asn1.x509.X509Certificate[] certificates = new org.bouncycastle.asn1.x509.X509Certificate[certs.length];
                        for (int i = 0; i != certificates.length; i++)
                        {
                            certificates[i] = org.bouncycastle.asn1.x509.X509Certificate.getInstance(certs[i].getEncoded());
                        }
        ...
        throws IOException, NoSuchAlgorithmException
    {
        ...
        try
        {
            if (storeEncryptionAlgorithm.equals(NISTObjectIdentifiers.id_aes256_CCM))
            {
                ...
                PBES2Parameters pbeParams = new PBES2Parameters(pbkdAlgId, new EncryptionScheme(NISTObjectIdentifiers.id_aes256_CCM, CmsCCMParameters.getInstance(algorithmParameters.getEncoded())));
        ...
        throws IOException, NoSuchAlgorithmException, CertificateException
    {
        ...
        else if (integrityCheck.getType() == ObjectStoreIntegrityCheck.SIG_CHECK)
        {
            ...
            try
            {
                org.bouncycastle.asn1.x509.X509Certificate[] certificates = sigCheck.getCertificates();
                if (validator != null)
                {
        ...
        throws IOException
    {
        ...
        try
        {
            ...
            if (algId.getAlgorithm().equals(NISTObjectIdentifiers.id_aes256_CCM))
            {
                ...
                CmsCCMParameters ccmParameters = CmsCCMParameters.getInstance(algId.getParameters());
