...
public final class AES
{
        ...
        extends BaseAlgorithmParameterGenerator
    {
        ...
        protected AlgorithmParameters engineGenerateParameters()
        {
            ...
            try
            {
                ...
                params.init(new CmsCCMParameters(iv, 12).getEncoded());
            }
            catch (Exception e)
            {
                throw new RuntimeException(e.getMessage());
    ...
    public static class AlgParamsCCM
        extends BaseAlgorithmParameters
    {
        private CmsCCMParameters ccmParams;
            ...
            throws InvalidParameterSpecException
        {
            if (GcmSpecUtil.isGcmSpec(paramSpec))
            {
                ccmParams = CmsCCMParameters.getInstance(GcmSpecUtil.extractGcmParameters(paramSpec));
            }
            else if (paramSpec instanceof AEADParameterSpec)
            {
                ccmParams = new CmsCCMParameters(((AEADParameterSpec)paramSpec).getNonce(), ((AEADParameterSpec)paramSpec).getMacSizeInBits() / 8);
            }
            else
            {
                throw new InvalidParameterSpecException("AlgorithmParameterSpec class not recognized: " + paramSpec.getClass().getName());
        ...
        protected void engineInit(byte[] params)
            throws IOException
        {
            ccmParams = CmsCCMParameters.getInstance(params);
        }
