...
import org.bouncycastle.jcajce.provider.asymmetric.util.JcaJceUtilDHUtil;
...
public class JcajceDhIESCipher
    extends CipherSpi
{
    ...
    public JcajceDhIESCipher(IESEngine engine)
    {
        this.engine = engine;
        this.ivLength = 0;
    }

    public JcajceDhIESCipher(IESEngine engine, int ivLength)
    {
        this.engine = engine;
        ...
        throws InvalidAlgorithmParameterException, InvalidKeyException
    {
        ...
        if (opmode == Cipher.ENCRYPT_MODE || opmode == Cipher.WRAP_MODE)
        {
            ...
            {
                this.key = JcaJceUtilDHUtil.generatePublicKeyParameter((PublicKey)key);
            }
            else if (key instanceof IESKey)
            {
                IESKey ieKey = (IESKey)key;

                this.key = JcaJceUtilDHUtil.generatePublicKeyParameter(ieKey.getPublic());
                this.otherKeyParameter = JcaJceUtilDHUtil.generatePrivateKeyParameter(ieKey.getPrivate());
            }
            else
            {
        ...
        else if (opmode == Cipher.DECRYPT_MODE || opmode == Cipher.UNWRAP_MODE)
        {
            ...
            {
                this.key = JcaJceUtilDHUtil.generatePrivateKeyParameter((PrivateKey)key);
            }
            else if (key instanceof IESKey)
            {
                IESKey ieKey = (IESKey)key;

                this.otherKeyParameter = JcaJceUtilDHUtil.generatePublicKeyParameter(ieKey.getPublic());
                this.key = JcaJceUtilDHUtil.generatePrivateKeyParameter(ieKey.getPrivate());
            }
            else
            {
