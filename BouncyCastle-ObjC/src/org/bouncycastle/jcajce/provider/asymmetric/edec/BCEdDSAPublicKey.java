    ...
    implements EdDSAKey, PublicKey
{
    ...
    public byte[] getEncoded()
    {
        ...
        {
            byte[] encoding = new byte[JcajceEdecKeyFactorySpi.Ed448Prefix.length + Ed448PublicKeyParameters.KEY_SIZE];

            System.arraycopy(JcajceEdecKeyFactorySpi.Ed448Prefix, 0, encoding, 0, JcajceEdecKeyFactorySpi.Ed448Prefix.length);

            ((Ed448PublicKeyParameters)eddsaPublicKey).encode(encoding, JcajceEdecKeyFactorySpi.Ed448Prefix.length);

            return encoding;
        }
        else
        {
            byte[] encoding = new byte[JcajceEdecKeyFactorySpi.Ed25519Prefix.length + Ed25519PublicKeyParameters.KEY_SIZE];

            System.arraycopy(JcajceEdecKeyFactorySpi.Ed25519Prefix, 0, encoding, 0, JcajceEdecKeyFactorySpi.Ed25519Prefix.length);

            ((Ed25519PublicKeyParameters)eddsaPublicKey).encode(encoding, JcajceEdecKeyFactorySpi.Ed25519Prefix.length);
    ...
    {
        return JcajceEdecUtils.keyToString("Public Key", getAlgorithm(), eddsaPublicKey);
    }
