...
class ECUtils
{
    ...
    {
        return (key instanceof BCECPublicKey) ? ((BCECPublicKey)key).engineGetKeyParameters() : JcajceUtilECUtil.generatePublicKeyParameter(key);
    }
    ...
    static X9ECParameters getDomainParametersFromName(String curveName)
    {
        ...
        try
        {
            if (curveName.charAt(0) >= '0' && curveName.charAt(0) <= '2')
            {
                ...
                domainParameters = JcajceUtilECUtil.getNamedCurveByOid(oidID);
            }
            else
            {
                if (curveName.indexOf(' ') > 0)
                {
                    ...
                    domainParameters = JcajceUtilECUtil.getNamedCurveByName(curveName);
                }
                else
                {
                    domainParameters = JcajceUtilECUtil.getNamedCurveByName(curveName);
