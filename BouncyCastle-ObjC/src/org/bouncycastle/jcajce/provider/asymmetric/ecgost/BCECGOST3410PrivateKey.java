...
import org.bouncycastle.jce.interfaces.JceECPrivateKey;
...
public class BCECGOST3410PrivateKey
    implements ECPrivateKey, JceECPrivateKey, PKCS12BagAttributeCarrier, ECPointEncoder
{
        ...
        throws IOException
    {
        ...
        else
        {
            ...
            if (params.isNamedCurve())
            {
                ...
                X9ECParameters ecP = JcajceUtilECUtil.getNamedCurveByOid(oid);
                ...
                {
                    curveName = JcajceUtilECUtil.getCurveName(oid);
                }
    ...
    public byte[] getEncoded()
    {
        ...
        else
        {
            ...
            {
                ASN1ObjectIdentifier curveOid = JcajceUtilECUtil.getNamedCurveOid(((ECNamedCurveSpec)ecSpec).getName());
                if (curveOid == null)  // guess it's the OID
                {
                    curveOid = new ASN1ObjectIdentifier(((ECNamedCurveSpec)ecSpec).getName());
                }
                params = new X962Parameters(curveOid);
                orderBitLength = JcajceUtilECUtil.getOrderBitLength(BouncyCastleProvider.CONFIGURATION, ecSpec.getOrder(), this.getS());
            }
            else if (ecSpec == null)
            {
                params = new X962Parameters(DERNull.INSTANCE);
                orderBitLength = JcajceUtilECUtil.getOrderBitLength(BouncyCastleProvider.CONFIGURATION, null, this.getS());
            }
            else
            {
    ...
    {
        return JcajceUtilECUtil.privateKeyToString(algorithm, d, engineGetSpec());
    }
