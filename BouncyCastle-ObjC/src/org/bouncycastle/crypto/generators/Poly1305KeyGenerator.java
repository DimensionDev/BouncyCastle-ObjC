...
/**
 * Generates keys for the JcajcePoly1305 MAC.
 * <p>
 * JcajcePoly1305 keys are 256 bit keys consisting of a 128 bit secret key used for the underlying block
 ...
 * A separately generated 256 bit key can be modified to fit the JcajcePoly1305 key format by using the
    ...
    extends CipherKeyGenerator
{
     ...
     * JcajcePoly1305 keys are always 256 bits, so the key length in the provided parameters is ignored.
     */
    public void init(KeyGenerationParameters param)
    {
        // JcajcePoly1305 keys are always 256 bits
        super.init(new KeyGenerationParameters(param.getRandom(), 256));
    }

    /**
     * Generates a 256 bit key in the format required for JcajcePoly1305 - e.g.
    ...
    /**
     * Modifies an existing 32 byte key value to comply with the requirements of the JcajcePoly1305 key by
    ...
    /**
     * Checks a 32 byte key for compliance with the JcajcePoly1305 key requirements, e.g.
     * <code>k[0] ... k[15], r[0] ... r[15]</code> with the required bits in <code>r</code> cleared
     * as per {@link #clamp(byte[])}.
     *
    ...
    private static void checkMask(byte b, byte mask)
    {
        if ((b & (~mask)) != 0)
        {
            throw new IllegalArgumentException("Invalid format for r portion of JcajcePoly1305 key.");
        }
    }
