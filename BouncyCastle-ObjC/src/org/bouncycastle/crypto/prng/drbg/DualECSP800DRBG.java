    ...
    implements SP80090DRBG
{
    ...
    public DualECSP800DRBG(DualECPoints[] pointSet, Digest digest, int securityStrength, EntropySource entropySource, byte[] personalizationString, byte[] nonce)
    {
        ...
        if (CryptoPrngDrbgUtils.isTooLarge(personalizationString, MAX_PERSONALIZATION_STRING / 8))
        {
        ...
        for (int i = 0; i != pointSet.length; i++)
        {
            ...
            {
                if (CryptoPrngDrbgUtils.getMaxSecurityStrength(digest) < pointSet[i].getSecurityStrength())
                {
        ...
        _s = CryptoPrngDrbgUtils.hash_df(_digest, seedMaterial, _seedlen);
        _sLength = _s.length;
    ...
    public int generate(byte[] output, byte[] additionalInput, boolean predictionResistant)
    {
        ...
        if (CryptoPrngDrbgUtils.isTooLarge(additionalInput, MAX_ADDITIONAL_INPUT / 8))
        {
        ...
        if (additionalInput != null)
        {
            ...
            additionalInput = CryptoPrngDrbgUtils.hash_df(_digest, additionalInput, _seedlen);
