    ...
    implements SP80090DRBG
{
    ...
    {
        if (securityStrength > CryptoPrngDrbgUtils.getMaxSecurityStrength(digest))
        {
        ...
        byte[] seed = CryptoPrngDrbgUtils.hash_df(_digest, seedMaterial, _seedLength);

        _V = seed;
        byte[] subV = new byte[_V.length + 1];
        System.arraycopy(_V, 0, subV, 1, _V.length);
        _C = CryptoPrngDrbgUtils.hash_df(_digest, subV, _seedLength);
