...
/**
 * JcajcePoly1305 message authentication code, designed by D. J. Bernstein.
 * <p>
 * JcajcePoly1305 computes a 128-bit (16 bytes) authenticator, using a 128 bit nonce and a 256 bit key
    ...
    implements Mac
{
    ...
    /** PqcMathPolynomial accumulator */
    private int h0, h1, h2, h3, h4;

    /**
     * Constructs a JcajcePoly1305 MAC, where the key passed to init() will be used directly.
     */
    public Poly1305()
    {
        this.cipher = null;
    ...
    /**
     * Constructs a JcajcePoly1305 MAC, using a 128 bit block cipher.
     */
    public Poly1305(final BlockCipher cipher)
    {
        if (cipher.getBlockSize() != BLOCK_SIZE)
        {
            throw new IllegalArgumentException("JcajcePoly1305 requires a 128 bit block cipher.");
        }
        this.cipher = cipher;
    }

    /**
     * Initialises the JcajcePoly1305 MAC.
     * 
     ...
     *        {@link Poly1305KeyGenerator JcajcePoly1305 key format}, otherwise just the
     *        {@link KeyParameter}.
     */
        ...
        throws IllegalArgumentException
    {
        ...
        if (cipher != null)
        {
            ...
            {
                throw new IllegalArgumentException("JcajcePoly1305 requires an IV when used with a block cipher.");
            }
        ...
        {
            throw new IllegalArgumentException("JcajcePoly1305 requires a key.");
        }
    ...
    private void setKey(final byte[] key, final byte[] nonce)
    {
        if (key.length != 32)
        {
            throw new IllegalArgumentException("JcajcePoly1305 key must be 256 bits.");
        }
        if (cipher != null && (nonce == null || nonce.length != BLOCK_SIZE))
        {
            throw new IllegalArgumentException("JcajcePoly1305 requires a 128 bit IV.");
        }
    ...
    {
        return cipher == null ? "JcajcePoly1305" : "JcajcePoly1305-" + cipher.getAlgorithmName();
    }
