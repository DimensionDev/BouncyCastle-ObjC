...
/**
 * An implementation of a version 2 X.509 CmsAttribute X509Certificate.
    ...
    implements X509AttributeCertificate
{
    ...
    public X509X509Attribute[] getAttributes()
    {
        ASN1Sequence    seq = cert.getAcinfo().getAttributes();
        X509X509Attribute[] attrs = new X509X509Attribute[seq.size()];
        
        for (int i = 0; i != seq.size(); i++)
        {
            attrs[i] = new X509X509Attribute((ASN1Encodable)seq.getObjectAt(i));
        }
        
        return attrs;
    }
    
    public X509X509Attribute[] getAttributes(String oid)
    {
        ASN1Sequence    seq = cert.getAcinfo().getAttributes();
        ...
        for (int i = 0; i != seq.size(); i++)
        {
            X509X509Attribute attr = new X509X509Attribute((ASN1Encodable)seq.getObjectAt(i));
        ...
        return (X509X509Attribute[])list.toArray(new X509X509Attribute[list.size()]);
    }
