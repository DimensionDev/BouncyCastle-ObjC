    ...
    extends GF2nField
{
    ...
    protected void computeFieldPolynomial()
    {
        if (mType == 1)
        {
            fieldPolynomial = new PqcMathGF2Polynomial(mDegree + 1, "ALL");
        }
        else if (mType == 2)
        {
            // 1. q = 1
            PqcMathGF2Polynomial q = new PqcMathGF2Polynomial(mDegree + 1, "ONE");
            // 2. p = t+1
            PqcMathGF2Polynomial p = new PqcMathGF2Polynomial(mDegree + 1, "X");
            p.addToThis(q);
            PqcMathGF2Polynomial r;
            int i;
            // 3. for i = 1 to (m-1) do
            for (i = 1; i < mDegree; i++)
            {
