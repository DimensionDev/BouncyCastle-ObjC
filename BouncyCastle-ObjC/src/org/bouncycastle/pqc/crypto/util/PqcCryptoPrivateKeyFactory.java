 ...
 */
public class PqcCryptoPrivateKeyFactory
{
    /**
    ...
    public static AsymmetricKeyParameter createKey(PrivateKeyInfo keyInfo) throws IOException
    {
        ...
        if (algOID.on(BCObjectIdentifiers.qTESLA))
        {
            ...
            return new QTESLAPrivateKeyParameters(PqcCryptoUtilUtils.qTeslaLookupSecurityCategory(keyInfo.getPrivateKeyAlgorithm()), qTESLAPriv.getOctets());
        }
        else if (algOID.equals(BCObjectIdentifiers.sphincs256))
        {
            return new SPHINCSPrivateKeyParameters(ASN1OctetString.getInstance(keyInfo.parsePrivateKey()).getOctets(),
                PqcCryptoUtilUtils.sphincs256LookupTreeAlgName(SPHINCS256KeyParams.getInstance(keyInfo.getPrivateKeyAlgorithm().getParameters())));
        }
        ...
        else if (algOID.equals(BCObjectIdentifiers.xmss))
        {
            ...
            PqcAsn1XMSSPrivateKey xmssPrivateKey = PqcAsn1XMSSPrivateKey.getInstance(keyInfo.parsePrivateKey());
        ...
        else if (algOID.equals(PQCObjectIdentifiers.xmss_mt))
        {
            ...
            try
            {
                PqcAsn1XMSSPrivateKey xmssMtPrivateKey = PqcAsn1XMSSPrivateKey.getInstance(keyInfo.parsePrivateKey());
