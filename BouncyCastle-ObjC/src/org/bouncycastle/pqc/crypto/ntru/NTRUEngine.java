...
import org.bouncycastle.pqc.math.ntru.polynomial.PqcMathPolynomial;
    ...
    implements AsymmetricBlockCipher
{
    ...
    private byte[] encrypt(byte[] m, NTRUEncryptionPublicKeyParameters pubKey)
    {
        ...
        while (true)
        {
            ...
            PqcMathPolynomial r = generateBlindingPoly(sData, M);
     ...
     */
    private PqcMathPolynomial generateBlindingPoly(byte[] seed, byte[] M)
    {
    ...
    private byte[] decrypt(byte[] data, NTRUEncryptionPrivateKeyParameters privKey)
        throws InvalidCipherTextException
    {
        PqcMathPolynomial priv_t = privKey.t;
        ...
        PqcMathPolynomial cr = generateBlindingPoly(sData, cm);
     ...
     */
    protected IntegerPolynomial decrypt(IntegerPolynomial e, PqcMathPolynomial priv_t, IntegerPolynomial priv_fp)
    {
        IntegerPolynomial a;
        if (params.fastFp)
        {
