...
class CommonFunction
{
    ...
    public static short load16(final byte[] load, int loadOffset)
    {
        ...
        if (load.length - loadOffset >= Const.SHORT_SIZE / Const.BC_BYTE_SIZE)
        {

            for (int i = 0; i < Const.SHORT_SIZE / Const.BC_BYTE_SIZE; i++)
            {

                number ^= (short)(load[loadOffset + i] & 0xFF) << (Const.BC_BYTE_SIZE * i);
    ...
    public static int load32(final byte[] load, int loadOffset)
    {
        ...
        if (load.length - loadOffset >= Const.INT_SIZE / Const.BC_BYTE_SIZE)
        {

            for (int i = 0; i < Const.INT_SIZE / Const.BC_BYTE_SIZE; i++)
            {

                number ^= (int)(load[loadOffset + i] & 0xFF) << (Const.BC_BYTE_SIZE * i);
    ...
    public static long load64(final byte[] load, int loadOffset)
    {
        ...
        if (load.length - loadOffset >= Const.LONG_SIZE / Const.BC_BYTE_SIZE)
        {

            for (int i = 0; i < Const.LONG_SIZE / Const.BC_BYTE_SIZE; i++)
            {

                number ^= (long)(load[loadOffset + i] & 0xFF) << (Const.BC_BYTE_SIZE * i);
    ...
    public static void store16(byte[] store, int storeOffset, short number)
    {
        ...
        if (store.length - storeOffset >= Const.SHORT_SIZE / Const.BC_BYTE_SIZE)
        {

            for (int i = 0; i < Const.SHORT_SIZE / Const.BC_BYTE_SIZE; i++)
            {

                store[storeOffset + i] = (byte)((number >> (Const.BC_BYTE_SIZE * i)) & 0xFF);
    ...
    public static void store32(byte[] store, int storeOffset, int number)
    {
        ...
        if (store.length - storeOffset >= Const.INT_SIZE / Const.BC_BYTE_SIZE)
        {
    ...
    public static void store64(byte[] store, int storeOffset, long number)
    {
        ...
        if (store.length - storeOffset >= Const.LONG_SIZE / Const.BC_BYTE_SIZE)
        {

            for (int i = 0; i < Const.LONG_SIZE / Const.BC_BYTE_SIZE; i++)
            {

                store[storeOffset + i] = (byte)((number >> (Const.BC_BYTE_SIZE * i)) & 0xFFL);
