    ...
    implements MessageSigner
{
    ...
    public byte[] generateSignature(byte[] message)
    {
        ...
        case QTESLASecurityCategory.HEURISTIC_I:
            PqcCryptoQTESLA.signingI(sig, message, 0, message.length, privateKey.getSecret(), secureRandom);
            break;
        case QTESLASecurityCategory.HEURISTIC_III_SIZE:
            PqcCryptoQTESLA.signingIIISize(sig, message, 0, message.length, privateKey.getSecret(), secureRandom);
            break;
        case QTESLASecurityCategory.HEURISTIC_III_SPEED:
            PqcCryptoQTESLA.signingIIISpeed(sig, message, 0, message.length, privateKey.getSecret(), secureRandom);
            break;
        case QTESLASecurityCategory.PROVABLY_SECURE_I:
            PqcCryptoQTESLA.signingIP(sig, message, 0, message.length, privateKey.getSecret(), secureRandom);
            break;
        case QTESLASecurityCategory.PROVABLY_SECURE_III:
            PqcCryptoQTESLA.signingIIIP(sig, message, 0, message.length, privateKey.getSecret(), secureRandom);
            break;
        default:
    ...
    public boolean verifySignature(byte[] message, byte[] signature)
    {
        ...
        {
        case QTESLASecurityCategory.HEURISTIC_I:
            status = PqcCryptoQTESLA.verifyingI(message, signature, 0, signature.length, publicKey.getPublicData());
            break;
        case QTESLASecurityCategory.HEURISTIC_III_SIZE:
            status = PqcCryptoQTESLA.verifyingIIISize(message, signature, 0, signature.length, publicKey.getPublicData());
            break;
        case QTESLASecurityCategory.HEURISTIC_III_SPEED:
            status = PqcCryptoQTESLA.verifyingIIISpeed(message, signature, 0, signature.length, publicKey.getPublicData());
            break;
        case QTESLASecurityCategory.PROVABLY_SECURE_I:
            status = PqcCryptoQTESLA.verifyingPI(message, signature, 0, signature.length, publicKey.getPublicData());
            break;
        case QTESLASecurityCategory.PROVABLY_SECURE_III:
            status = PqcCryptoQTESLA.verifyingPIII(message, signature, 0, signature.length, publicKey.getPublicData());
            break;
        default:
