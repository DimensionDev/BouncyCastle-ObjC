        ...
        if (keySpec instanceof X509EncodedKeySpec)
        {
            ...
            try
            {
                if (PQCObjectIdentifiers.mcElieceCca2.equals(pki.getAlgorithm().getAlgorithm()))
                {
                    ...
                    return new BCMcElieceCCA2PublicKey(new McElieceCCA2PublicKeyParameters(key.getN(), key.getT(), key.getG(), PqcJcajceMcelieceUtils.getDigest(key.getDigest()).getAlgorithmName()));
                }
                else
                {
        ...
        if (keySpec instanceof PKCS8EncodedKeySpec)
        {
            ...
            try
            {
                if (PQCObjectIdentifiers.mcElieceCca2.equals(pki.getPrivateKeyAlgorithm().getAlgorithm()))
                {
                    ...
                    return new BCMcElieceCCA2PrivateKey(new McElieceCCA2PrivateKeyParameters(key.getN(), key.getK(), key.getField(), key.getGoppaPoly(), key.getP(), PqcJcajceMcelieceUtils.getDigest(key.getDigest()).getAlgorithmName()));
                }
                else
                {
